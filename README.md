...
## Files
- `index.html` + `style.css` + `app.js` — main site (feed, posting, reactions, comments, embeds, mentions, follow, side menu)
- `profile.html` — user profile pages (stats, follow button, their posts)
- `friends.html` — list of people you follow, with unfollow
- `notifications.html` — posts/comments that @mention you
- `guidelines.html`, `features.html`, `privacy.html`, `terms.html` — static info pages, linked from the side menu
- `admin.html` — moderator-only panel (visitors, analytics, broadcast, posts, banned words, users, bans, titles, sub-mods)
- `schema.sql` — full schema for a **brand-new** Supabase project
- `fix-and-upgrade.sql`, `upgrade-v2.sql` through `upgrade-v7.sql` — patches for an **already-deployed** project, run in order
...
6. `upgrade-v6.sql` — fixes "Restore Account" failing (missing DELETE policy on users).
7. `upgrade-v7.sql` — adds the announcement/broadcast flag.
...
4. Paste those into the top of `app.js` (`SUPABASE_URL`, `SUPABASE_ANON_KEY`) **and** into `admin.html`, `profile.html`, `friends.html`, and `notifications.html` (each has the same two constants near the top of its `<script>` block).
...
- **Delete dead users**: `admin.html`'s user list now shows "Last seen" for each account, and a "Delete User" button (full moderator only) permanently removes an account along with all their posts, comments, likes, and follows. Can't be undone.
- **Analytics**: `admin.html` → 📊 Analytics tab shows total users, live post count, posts/comments in the last 24h, banned-user count, sub-moderator count, and the 5 most active users by post count.
- **Broadcast/Announcements**: `admin.html` → 📢 Broadcast tab (full moderator only) lets you post an official announcement — it appears at the very top of the feed with a highlighted purple border and a "📢 Announcement" badge, stays pinned, and is exempt from the 36-hour auto-delete. Remove one anytime from the same tab.
- **Side menu**: tap ☰ next to "The Court" title to open a slide-out menu with Home, your Profile, Friends, Notifications, Community Guidelines, Features/How To Use, Privacy Policy, Terms, and Admin (the Admin link only shows if you're a moderator or sub-moderator).
- **Friends page**: shows everyone you follow, with a quick unfollow button.
- **Notifications page**: shows recent posts/comments where someone @mentioned you.
