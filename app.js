...
async function toggleFollow(targetUserId, currentlyFollowing) {
  if (currentlyFollowing) {
    await sb.from("follows").delete().eq("follower_id", CURRENT_USER.id).eq("followee_id", targetUserId);
  } else {
    await sb.from("follows").insert({ follower_id: CURRENT_USER.id, followee_id: targetUserId });
  }
  loadFeed();
}
