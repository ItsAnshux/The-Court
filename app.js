...
async function loadFeed() {
  ...
  feed.innerHTML = posts.map(p => postTemplate(p, likedSet.has(p.id))).join("");
  ...
  <div class="post" data-id="${p.id}">
    ...
    <div class="post-body">
      ...
      <div class="post-actions">
        ...
        <div class="action comment-toggle" data-id="${p.id}">💬 <span class="comment-count-${p.id}"></span></div>
...
async function toggleLike(postId) {
  ...
  if (isLiked) {
    await sb.from("likes").delete().eq("post_id", postId).eq("user_id", CURRENT_USER.id);
...
(async function boot() {
  ...
  await loadFeed();
  setupPresence();
  setInterval(loadFeed, 20000); // refresh feed every 20s
})();
