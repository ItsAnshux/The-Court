...
function setupSideMenu() {
  ...
  const profileLink = document.getElementById("menuProfileLink");
  if (profileLink) profileLink.href = `profile.html?u=${CURRENT_USER.id}`;

  const adminLink = document.getElementById("menuAdminLink");
  if (adminLink) adminLink.style.display = (CURRENT_USER.is_moderator || CURRENT_USER.is_submod) ? "block" : "none";

  function openMenu() { overlay.style.display = "block"; menu.style.left = "0"; }
  function closeMenu() { overlay.style.display = "none"; menu.style.left = "-280px"; }
...
async function restoreAccount() {
  ...
  const { data: delData, error: delError } = await sb.from("users").delete().eq("id", oldGuestId).select(); // frees up this device's fingerprint
  if (delError) { alert("Purana guest account delete nahi hua: " + delError.message); return; }
  if (!delData || delData.length === 0) { alert("Purana guest account delete nahi ho paya (permission issue) — restore rok diya gaya."); return; }
...
function postTemplate(p, myReaction, commentCount, isFollowing) {
  ...
  const titleBadge = u.title
    ...
    : "";
  const pinBadge = p.pinned && !p.is_announcement ? `<span class="title-badge" style="color:var(--gold);border-color:var(--gold);">📌 Pinned</span>` : "";
  const announceBadge = p.is_announcement ? `<span class="title-badge" style="color:#fff;background:var(--accent-grad);border:none;">📢 Announcement</span>` : "";
  ...
  return `
  <div class="post" data-id="${p.id}" style="${p.is_announcement ? "border-color:var(--accent);background:linear-gradient(180deg, #7c5cff14, var(--panel));" : ""}">
